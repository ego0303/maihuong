const CONFIG = {
    titleWeb: "Mia",
    introTitle: 'Miaaaaa',
    introDesc: `Ngay khi người đẹp nhìn thấy dòng chữ vô tri này, Vũ trụ đang nói với tui rằng hãy nhắc người đẹp đặt điện thoại xuống, tắt đèn và đi ngủ đi.`,
    btnIntro: 'Tiếp nào',
    title: 'heyy heyy người đẹp, chị đã có một ngày dài và mệt mỏi, đã đến lúc hãy tạm quên mọi chuyện mà đi nghỉ ngơi đi nhen🌙',
    desc: 'Nếu đọc được đến dòng này thì đi ngủ đi, em hết văn chương để viết ời :))',
    btnYes: 'Đi ngủ',
    btnNo: 'Hỏng ngủ))',
    question: 'Tui biết mấy  bất lực lắm mới bấm nút đi ngủ chứ gì, biết hết!!😏',
    btnReply: 'Gửi',
    reply: 'Đi ngủ đi ngủ không ngủ cũng tắt điện thoại mà lên giường nằm😌😌',
    mess: 'Đó là những gì một cô géi ngoan nên làm:))',
    messDesc: 'Mai mua cho ly sữa:))',
    btnAccept: 'Ok luôn nà',
    messLink: 'https://www.youtube.com/watch?v=9TSPbfbJUkQ' //link mess của các bạn. VD: https://www.facebook.com/messages/t/100014188333536
}
